...
const DesktopNavBar = ({ theme, auth }) => {
    const { isAuth } = auth;
    const navLinkClasses = theme === 'light' ? styles.nav__link : styles.nav__link + ` ${styles['nav__link-dark']}`;
    return (
        <header className={theme === 'light' ? styles.header : `${styles.header} ${styles['header-dark']}`}>
